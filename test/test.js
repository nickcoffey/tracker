...
describe('DB Utils', () => {
    ...
    describe('formatInsertQuery', () => {
        it('should return 1 field and 1 value in query', () => {
            ...
            expect(output).to.be.equal('INSERT INTO category(name) VALUES($1) RETURNING id,name')
        })
        it('should return multiple fields and values in query', () => {
            var output = dbUtils.formatInsertQuery('category', ['name', 'description'])
    ...
    describe('formatSelectAllWhereQuery', () => {
        it('should return a query with 1 column parameter', () => {
            var output = dbUtils.formatSelectAllWhereQuery('category', ['id'])
            expect(output).to.be.equal('SELECT * FROM category WHERE id=$1')
        })

        it('should return a query with multiple column parameters', () => {
            var output = dbUtils.formatSelectAllWhereQuery('category', ['name', 'description'], ['test', 'qwerty'])
    ...
    describe('formatUpdateWhereQuery', () => {
        it('should return a query with 1 column parameter', () => {
            ...
            expect(output).to.be.equal('UPDATE category SET name=$1,description=$2 WHERE id=$3 RETURNING id,name,description')
        })

        it('should return a query with multiple column parameters', () => {
            var output = dbUtils.formatUpdateWhereQuery('category', ['name', 'description'], ['name', 'description'])
